...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(WindTurbineModel_V2_M, 0);
  ...
  WindTurbineModel_V2_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(WindTurbineModel_V2_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (WindTurbineModel_V2_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(WindTurbineModel_V2_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(WindTurbineModel_V2_M);
